...
public class ObjectCloneUtils {
    ...
    public static Object deepClone(Object objSource) throws InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        ...
        Object objDes = clazz.getDeclaredConstructor().newInstance();
        // 获得源对象所有属性
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                if (type.isArray()) {//对数组类型的字段进行递归过滤
                    ...
                    Collection newCollection = (Collection) value.getClass().getDeclaredConstructor().newInstance();
                    ...
                    Map newMap = (Map) value.getClass().getDeclaredConstructor().newInstance();
                    Map<?, ?> m = (Map<?, ?>) value;
                    Set<?> set = m.entrySet();
                    for (Object o : set) {
